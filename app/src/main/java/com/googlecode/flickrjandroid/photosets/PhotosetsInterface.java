...
public class PhotosetsInterface {
	...
	public PhotosetsInterface(String apiKey, String sharedSecret,
	 ...
	 * <p/>
	 * Note: requires authentication with the new authentication API with
	 * 'write' permission.
	 * 
	 * @param photosetId
	 *            The photoset ID
	 ...
	 */
	public void addPhoto(String photosetId, String photoId) throws IOException,
	...
	public Photoset create(String title, String description,
		...
		JSONObject photosetElement = response.getData().getJSONObject(
				"photoset");
	 ...
	 * @throws JSONException
	 */
	public void delete(String photosetId) throws IOException, FlickrException,
			JSONException {
		List<Parameter> parameters = new ArrayList<Parameter>();
	 ...
	 * @throws JSONException
	 */
	public void editPhotos(String photosetId, String primaryPhotoId,
			String[] photoIds) throws IOException, FlickrException,
			JSONException {
		List<Parameter> parameters = new ArrayList<Parameter>();
		...
		parameters.add(new Parameter("photoset_id", photosetId));
		parameters.add(new Parameter("primary_photo_id", primaryPhotoId));
		parameters.add(new Parameter("photo_ids", StringUtilities.join(
				photoIds, ",")));
		OAuthUtils.addOAuthToken(parameters);
	...
	private Photoset parsePhotoset(JSONObject photosetElement)
			throws JSONException {
		Photoset photoset = new Photoset();
		photoset.setId(photosetElement.getString("id"));
		...
		primaryPhoto.setSecret(photosetElement.optString("secret", null)); // TODO
																			// verify
																			// that
																			// this
																			// is
																			...
																			// the
																			// photo
		primaryPhoto.setServer(photosetElement.optString("server", null)); // TODO
																			// verify
																			// that
																			// this
																			// is
	 ...
	 * 
	 * This method does not require authentication. But to get a Photoset into
	 * the list, that contains just private photos, the call needs to be
	 * authenticated.
	 * 
	 * @param userId
	 *            The User id
	 ...
	 * Convenience method.
	 * 
	 * Calls getPhotos() with Extras.MIN_EXTRAS and
	...
	public Photoset getPhotos(String photosetId, int perPage, int page)
			throws IOException, FlickrException, JSONException {
		return getPhotos(photosetId, Extras.MIN_EXTRAS,
				Flickr.PRIVACY_LEVEL_NO_FILTER, perPage, page);
	}
	 ...
	 * @throws JSONException
	 */
	public void orderSets(String[] photosetIds) throws IOException,
			FlickrException, JSONException {
		List<Parameter> parameters = new ArrayList<Parameter>();
		...
		parameters.add(new Parameter("photoset_ids", StringUtilities.join(
				photosetIds, ",")));
		OAuthUtils.addOAuthToken(parameters);
